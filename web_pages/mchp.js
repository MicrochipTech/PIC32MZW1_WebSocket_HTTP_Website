...
function pollAJAX() {
	...
	for(i = ajaxList.length; i > 0; i--)
	{
		...
		if(curAjax.ajaxReq.readyState == 4 && curAjax.ajaxReq.status == 200) {
			...
			if((typeof(curAjax.container) == 'function') && (curAjax.ajaxReq.responseXML != null)){
...
function getSecurity(sec) {
	...
	case 3: // support WPA2
		security = "wpa1";
		break;
	case 4: // support both WPA and WPA2
		security = "wpa2";
		break;
	case 5: // support WPA3
	case 6: // support WPA3
...
function securitySsidPrompt() {
	var ssid;

	var errMsg;
	do {
		msg = "Please enter the name(SSID) for the network";
...
function infraSel() {
	if (document.getElementById('sec1').options.length == 2) {
		document.getElementById('sec1').options[2] = new Option('WPA-PSK', '4');
		document.getElementById('sec1').options[3] = new Option('WPA2-PSK', '3');
	}
}
			...
			if (currBss != getXMLValue(xmlData, 'count', 0)) {
				if (getXMLValue(xmlData, 'valid', 0) === '0') {
					...
					else
					{
						if(getXMLValue(xmlData, 'name', 0) == null)
						{
							// display hidden networks here as ""
							addScanRow("", getXMLValue(xmlData, 'privacy', 0), getXMLValue(xmlData, 'strength', 0), getXMLValue(xmlData, 'wlan', 0));
						}
					}
